...
ComPtr<ID3D12Resource> Utils::CreateDefaultBuffer(ID3D12Device* device, ID3D12GraphicsCommandList* CmdList,
	const void* initData, UINT64 byteSize, ComPtr<ID3D12Resource>& uploadBuffer) 
{
	...
	Fail(device->CreateCommittedResource(&heapPr, D3D12_HEAP_FLAG_NONE, &bs,
...
UINT Utils::CalcConstantBufferSize(UINT byteSize) {
